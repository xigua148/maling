...
### Fixed
- **设置误操作**：鼠标经过下拉框、数值框与滑块时滚轮不再误改值，滚轮会继续滚动设置页面。
- **聊天气泡空白**：滚轮不再滚动气泡内文档，避免文字偏移和气泡下方留下空白。
- **Windows 11 毛玻璃不可见**：修复 Mica 材质被主窗结构背景遮住、样式未完整重刷，以及低窗口透明度与 DWM 材质冲突的问题；不支持环境仍安全回退纯色。
- **主题跟随不完整**：首页、设置、回忆、记忆中心及聊天局部控件会随主题/深浅模式刷新；清理遗留硬编码色。
- **关系等级延迟生效**：聊天加分后立即更新 system prompt、称呼和口吻；升级事件只触发一次并驱动情绪/成就；定制角色不再覆盖关系规则。
- **侧栏小形象不跟随角色**：大形象下方的入口改为“角色姓名 · 好感度阶段”，优先展示当前角色专属表情资产，缺失时才回退默认资产。
- **控件可读性**：设置页外观下拉框统一圆角/字体/高度；角色页角色名称、描述、名字输入框在各主题下均保持文字与占位符可见。

### Changed
- 发布资产命名调整为 `Desktop.zip` / `Portable.exe`，并在 Release 页面提供中文显示名与下载指引。
- 新增 `NOTICE`，补齐 Pi Coding Agent Runtime、小游戏、字体、图标及运行时依赖的第三方归属/许可登记；角色表情差分按项目自有 AI 生成素材分发。
