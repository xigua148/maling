...
## v2.5.0 — 记忆链路正确性收口 + 三处能力补齐 + 配置路径根治

**版本**：**2.5.0**（2026-09-19）。**源码侧改动，尚未出构建包**（`version.json` 由发布流程回填）。

> **关于内置酒馆**：自本版起，内置酒馆**标记为弃用，只修不增**。原计划中的「T-20 记忆钩子」（把酒馆剧情里的物件写进日后闲聊）一并取消。已装版本不受影响，功能照常可用，但不再投入优化。

### Fixed

- **加载旧会话会用旧快照覆盖全局记忆与亲密度**：`session.py` 的 `load()` 与 `utils.py` 的崩溃恢复路径都会调 `memory_mgr.from_dict(...)` / `intimacy.from_dict(...)`。而 `memory` 与 `intimacy` 都是**全局单例数据**（各有一个权威文件），会话里嵌的那份是「存这个会话当时的快照」——加载旧会话等于**用三周前的状态覆盖至今的全部记忆**（`from_dict` 内部还会落盘），是数据回退而非恢复。现已**取消这两处回放**；会话文件仍保留这两个键（存档价值：可人工查证当时状态），只是不再应用。
...
- **话题提及注入通道**：补上双通道体系的最后一块缺口。此前实体、回应规则、影像记忆、情绪都有「提及命中注入」，**唯独话题只有通道①的静态 top-3 快照**——话题一旦归档或掉出前三，用户后来再聊起它也唤不醒。现在用户消息命中话题名时，当轮注入一行「【之前聊过】主人提过「X」（N 天前），已经完成了；…」（每轮 ≤2 条、≤150 字符）。**active 与 archived 都搜**，归档话题也能被自然唤起。
  - 匹配刻意**保守**：只做整串子串 + 空白分词（「学 Rust」在你说「Rust」时能命中），**不做中文前缀模糊匹配**（「装修计划」不会因为你说「装修」就被唤起）。该通道每轮都跑，宁可漏、不可误注入。
- **话题完成自动检测**：说「Rust 搞定了」「考研复习考完了」这类句子时，会自动把对应话题标记完成并归档（写 `completed_at`、移入已归档区）。
  - **核心守护**：命中完成句式**且同时点名了话题**才归档。只说「今天搞定了好多事」而手里有别的活跃话题时，**绝不猜测**——误归档让话题凭空消失，比漏归档严重得多。
- **提取词表外挂热补**：新增 `~/.maid_coder/memory_patterns.json`（与记忆文件同目录），可追加偏好/话题正则、完成句式、情绪关键词。**追加式合并**（内置优先级不受影响），**改文件即时生效**（mtime 变化自动重载，不必重启），**文件损坏静默回落内置**（绝不因词表炸掉提取链）。

### Changed

- **MemoryManager 增加进程内锁**（可重入锁，`_load` / `_save` 全程持锁）。背景：GUI 主线程写记忆的同时，日记 daemon 线程会读记忆；且 v2.4 接通捕获后写入频率由「几乎不写」变为「每轮都写」。
  - **边界说明**：只保证**进程内**。跨进程（CLI 与 GUI 同时运行）仍是「末次写者胜出」，由「唯一临时名 + replace 重试 + `.bak.1` 快照」三层兜底。

### Removed

- **死代码 `MemoryManager.update_topic_status`**：全仓零调用（连测试都没有）。它的语义是「只改状态、不移出活跃列表」，与 `complete_topic`（改状态 + 移入已归档）重叠，保留只会让后续维护者误用它造出「活跃列表里躺着已完成话题」的脏状态。

### Notes

- **零新增第三方依赖**，**零数据格式变更**（外挂词表是独立文件，不进 `user_memory.json` 的 schema）。
...
## v2.4.0 — 记忆系统：接通捕获链路 + 续接排序升级

**版本**：**2.4.0**（2026-09-18）。**源码侧改动，尚未出构建包** —— `version.json` 的 `assets.*.sha256/size` 由 `tools/build_release.py` 在发布时回填，本版未跑发布流程。

### Fixed

- **GUI 记忆捕获链路从未接通（本版主要修复）**：`extract_from_dialogue`（偏好/话题/情绪自动提取）与 `archive_stale_topics`（过期话题归档）此前**只有 CLI 调用**（`session.py:595` / `:600`），GUI 链路一次都不调。后果是 GUI 用户的记忆**永不积累**——实测 `~/.maid_coder/user_memory.json` 自 2026-08-29 首启生成默认骨架后**三周零写入**，偏好与话题全空（同期 `companion.json` / `sessions/` 均活跃，可排除「用户没用」）。
  - 连带影响：`pick_topic_followup()` 恒返回 `None`（**主动续接从不触发**）、`build_memory_context()` 恒返空串（system prompt 无记忆段）、记忆中心的「自动提取」角标在 GUI 下**永不可能出现**（无任何代码能在 GUI 产出 `source="auto"`）。
  - 修复方式：在 `_on_stream_finished` 的非群聊分支新增捕获钩子 `_extract_memory_after_reply`，守卫纪律与既有 `_stash_vision_memory` 一致（demo / 取消 / 暂存缺失 / 记忆管理器缺失一律跳过；异常只记 debug 日志，**绝不阻塞消息链**）。
  - **群聊三隔离（R-J④⑤ / 共享知识 26）原样保住**：挂载点位于群聊分流 `return` 之后，结构性成立，既有 `TestTripleIsolation` 无需改动。
- **影像记忆取回首条错位**：`list_vision_memories` 用 `sort(reverse=True)` 排时间倒序，而 Python 稳定排序对**相等元素保持插入序**；`datetime.now()` 在 Windows 计时器分辨率下会撞时间戳（实测 55 条仅 34 个不同值），撞车时「最新一条在前」失效。改为「升序稳定排序 + 整体 reverse」。该缺陷表现为 `test_v18_batch4.py::test_rolling_eviction_keeps_newest_50` **单跑必过、批跑必挂**。
- **记忆文件损坏时会静默丢数据**：`_load()` 原遇 `JSONDecodeError` 直接 `pass`，随即用默认结构**覆盖写盘**，无备份、无日志——这是本模块唯一会静默销毁用户记忆的路径。现按项目既有约定（`gui/tavern/store.py` 的 L5/L6）补齐：写前快照 `.bak.1`（**复制**而非移动，写失败原档不被破坏）→ 主档坏则从快照恢复 → 快照亦坏则把坏档**改名保留**为 `.corrupt[.n]`（**绝不静默删除**）→ 才回落默认结构。
- **话题提取误捕垃圾**：`_TOPIC_PATTERNS` 第三条的分隔词原为可缺省，导致「这个项目做完了」被捕获成话题「做完了」。现加前导锚点 + 分隔符必选 + 话题长度上限收到 30 字（实测 4 个垃圾场景全挡住、6 个正常表达全保留）。

### Changed

- **话题续接排序由「计数字典序」改为多因子评分**：原排序键 `(followup_score, 列表序)` 意味着一个被点过「💗 说到心坎」的话题在 72 小时窗内**永远**压过其他所有话题、与时间无关，且计数无上限会长期霸榜。新评分 `0.40*有用信号 + 0.40*时间近因 + 0.20*来源可信度`，时间项为**真半衰期**（36h 处权重恰为 0.5，72h 窗边缘 0.25）。
  - 效果：3 小时前刚提到的（0.498）现在会胜过 60 小时前被夸过一次的（0.406）；12 小时内被夸过两次的（0.677）仍胜过 2 小时前随口提到的（0.585）。
  - 三条硬门（「先不提」静默 / 7 天去重 / 时间窗）与 `followup_score` 字段语义**均未改动**。

### Notes

- **零新增第三方依赖**，**零数据格式变更**：新评分为纯内部计算量，不落任何字段，因此不涉及读时迁移、不涉及 `meta.version`。
- 本版评估的两个开源项目**均未并入代码**：LingChat（AGPL-3.0，且 v0.5.0 主动移除了对外通信层，无法以「未修改的独立进程」形态接入——见 `docs/THIRD_PARTY.md` 的许可白名单与豁免失效边界）；Autonomous-Long-Term-Memory-System（Apache-2.0 可并入，但只吸收其设计、不 vendor 源码——理由见 `docs/prd-v24.md` §4 N2）。
- 回归：**2397 passed / 1 failed / 9 skipped**。唯一失败 `test_v20_updater.py::test_enumerate_finds_own_process` 为**既有环境问题**（Windows venv 的 `python.exe` 重定向器使进程真实 image 路径不在被枚举的盘符下），单独跑即失败（0.18s），其依赖的 `maling_updater.py` 本版零改动。
- 设计文档：`docs/prd-v24.md` + `docs/design-v24.md`。

---

## v2.3.1 — 便携版瘦身

**版本**：**2.3.1**（2026-09-17）。**分发**：`MaLing_v2.3.1_Desktop.zip`（桌面版，解压即用，**含内置酒馆**）与 `MaLing_v2.3.1_Portable.exe`（单文件版，免解压，**不含 Pi 编程引擎与内置酒馆**），各附 SHA-256 校验文件。

本版**只改打包配置，无代码逻辑变更**。

### Changed

- **便携版（Portable.exe）不再内嵌 SillyTavern 源码树**：便携版不内置 Pi 运行时（既有裁决 Q-U10 → 无需对 onefile 跑 `copy_pi_runtime.py`），而内置酒馆的启动依赖其中的 `node.exe` —— 因此那 292 MB 在便携版里**完全用不上**，点开该入口只会得到一条可读的失败提示。排除后**体积由约 500 MB 回落到约 200 MB**；失败行为不变（提示语由「找不到捆绑的 node.exe」变为「找不到内置的 sillytavern 目录」，两者都是可读文案），桌面版不受影响。

### Notes

- **便携版不含 Pi 编程引擎，也不含内置酒馆**；需要这两项请使用桌面版 zip。
- 本版未做代码签名，Windows 可能提示「未知发布者」。

---

## v2.3.0 — 内置 SillyTavern

**版本**：**2.3.0**（2026-09-17）。**分发**：`MaLing_v2.3.0_Desktop.zip`（桌面版，解压即用，**含内置酒馆**）与 `MaLing_v2.3.0_Portable.exe`（单文件版，免解压，**不含内置酒馆**，见 Notes），各附 SHA-256 校验文件。

### Added

- **内置 SillyTavern**：侧栏新增「Silly Tavern」入口，把官方 SillyTavern（1.19.0）作为**独立的本地 Node 服务**内嵌进码铃，点开即用，用户无需自行安装 Node 或 npm。
  - **原样捆绑**：上游源码逐字节未改动；以独立进程运行，仅在本机回环（`127.0.0.1`）提供服务，内嵌浏览器经本机 HTTP 访问 —— 不与码铃代码同进程、不链接其代码。
  - **数据隔离**：酒馆的数据与配置全部落在 `%APPDATA%\maling\sillytavern\`，程序目录（含安装目录）**零写入**，升级不丢档。
  - **旧「酒馆」原样保留**：项目自研的轻量剧情引擎不受影响，两个入口各自独立。
- **启动预热**（默认开，设置页「内置酒馆」可关）：应用启动约 1 分钟后在后台预启服务，点开时通常已就绪。冷启动需 15~50 秒（其中编译前端是每次启动的固定成本），不预热则需对着加载态等待。
- **三段式进度**：启动过程显示「准备运行时 → 初始化内容与前端 → 启动服务」，不再只有一个笼统的等待提示。
- **大屏模式**：酒馆就绪后可用「在浏览器中打开」脱离应用窗口使用。
- **诊断包导出**（设置页「诊断」）：一键收集应用日志、酒馆服务端输出、环境与进程树、**脱敏后**的配置，打包成一个 zip；反馈问题时不必再手工凑材料。

### Fixed

- **内嵌酒馆界面不显示**（只显示顶部一条黑栏、主体空白）：`QWebEngineView` 被布局末尾的空白项压成了 **0 高度** —— WebEngine 本身全程正常（进程、DOM、HTTP、页面脚本均无异常），纯粹是视口高度为 0。现已让视图独占剩余空间，界面完整显示。
- **中文路径下的 Node 递归复制缺陷**：Node 在含中文或非 ASCII 字符的路径下执行递归目录复制时会**静默硬崩溃**（无任何错误输出），或更隐蔽地**静默不复制**（内容缺失且永不重试）。上游 issue `nodejs/node#54476`；已随包附带运行时兼容补丁，覆盖 SillyTavern 全部 **12 处**调用点，且 `vendor` 源码树保持逐字节不变。中文 Windows 用户首启不再失败。
- **孤儿进程根治**：酒馆的 Node 进程此前只在应用"走到清理代码"时才被停掉；若应用崩溃、被强制结束，或系统关机时窗口处于最小化/托盘状态，进程会残留。现改为把子进程纳入 Windows 作业对象，由**内核**保证随码铃一起回收。
- **开发/构建侧**（不影响使用者）：补齐测试依赖声明（缺 `pytest-qt` 时全部 GUI 用例会报 ERROR 而非 failed，读数静默失效）；两个打包 spec 的配置模板改为"缺失时回退到入库的 `config.example.yaml`"，使他人从仓库克隆后可直接构建。

### License

- SillyTavern 采用 **AGPL-3.0**，作为「纯外部运行时依赖」（与 PySide6/LGPL 同口径）经主理人签发豁免后纳入：随包附**完整许可全文**（`_internal/sillytavern/LICENSE`），并在仓库根 `NOTICE` 第五节、`docs/THIRD_PARTY.md`、`docs/third_party_licenses/` 与「关于」页四处登记版本锚点与未修改声明。

### Notes

- **便携版不含内置酒馆**：单文件版 `Portable.exe` 不内置 Pi 运行时（既有裁决 Q-U10），而内置酒馆的启动依赖其中的 `node.exe` —— 点开该入口会给出可读提示「找不到捆绑的 node.exe」，不会崩溃。需要内置酒馆请使用桌面版 zip。
- 本版**无破坏性变更**，旧版数据与配置全部沿用。
- 本版**未做代码签名**，Windows 可能提示「未知发布者」。

---

## v2.2.2 — 女仆立绘修复（围裙下摆缺失）

**版本**：**2.2.2**（2026-09-17）。**分发**：`MaLing_v2.2.2_Desktop.zip`（桌面版，解压即用）与 `MaLing_v2.2.2_Portable.exe`（单文件版，免解压），各附 SHA-256 校验文件。

本版**只改了一张立绘，无代码逻辑变更**。

### Fixed
- **女仆立绘的围裙下摆缺了一块**（`gui/assets/maid/normal.png`）：深色主题下裙摆处直接透出底色、呈一块被啃掉的缺口；浅色主题下白对白，几乎看不出，因此长期未被发现。
  - **根因**：这批立绘的透明化是后来用脚本**按颜色阈值全局删白**做的，而**围裙本身就是白色** ⇒ 被轮廓包住的白色被**整块删掉**。与 2026-09-14 记录过的母题同源（当时是鲸鱼娘那批）。
  - **量化判据**（深底渲染 + 「底边整行不透明像素占比」）：破损版 **23.05%** → 完整版 **58.59%**；内部空洞由「下摆一大块」降为「仅边缘细游丝」。
  - **范围复核**：其余 **35 张**女仆表情的 `≥1200px` 内部空洞为 **0 张** ⇒ 本次只改这一张。

### Changed
- 两个测试文件里取证路径注释随工作台目录改名同步订正（`_release_v220` → `_release_work`），避免留下悬空路径。

## v2.2.1 — 可辨识性修复 + 更新链路修复

**版本**：**2.2.1**（2026-09-16）。**测试**：**2229 passed / 9 skipped / 1 failed**（唯一失败为**预期内**的 `tests/test_v20_assets.py::TestVersionJsonContract::test_assets_contract` —— 它断言 `version.json` 的 `assets.*.size > 0`，而该字段要等**本版构建完成**后回填真实 sha256/size 才非零。填完即绿）。

**分发**：`MaLing_v2.2.1_Desktop.zip`（桌面版，解压即用）与 `MaLing_v2.2.1_Portable.exe`（单文件版，免解压），各附 SHA-256 校验文件。

本版**没有新功能**，只修缺陷；其中最重的一条是「检查更新」长期失效的根因。

### Fixed
- **更新检查一直失败（根因）**：更新清单地址写在 `main` 分支，而本仓库默认分支自始至今是 `master` → 线上 **404** → **v2.0.0 起所有已装客户端的「检查更新」都拿不到结果**（静默失败）。已改为指向 `master`。
  - 该地址是**构建期写死**在程序里的，所以**已装的 v2.0.0～v2.2.0 改不了自己** —— 它们的快照读的仍是 `main`。因此 `main` 继续作为**镜像分支**每版同步推送（发版流程已改为双推 + 双 raw 验证），否则那批用户会再次静默收不到更新。
- **beta 频道恒 404**：设置页把「测试版（beta）」作为可选频道暴露给用户，但仓库从未提供 `beta/version.json` → 选 beta 的用户「检查更新」必然失败。已补出该清单（当前无独立 beta 构建，按 `beta ≥ stable` 约定镜像 stable）。
- **独立浮窗输入框的焦点圈完全不可见**：焦点圈取 `accent_light`，而四套主题里 `accent_light ≡ bg_light`，且该输入框自身底也是 `bg_light` → 焦点圈与自身底对比 **1.000**。改取 `accent_text` 后 4.125。
- **输入框 / 下拉 / 复选框焦点圈对比度不足**（WCAG 2.1 非文本对比需 ≥3:1）：四套主题最差 **1.931**（奶油浅色）→ 全部 ≥4.12；另含 `base.qss` 与主题 qss 共 6 处同类残留。
- **列表 / 页签选中态边界不可辨**（奶油主题 **2.102**）：加 1px `accent_text` 描边环 → **4.924**，并用 `padding −1px` 补偿保证**选中前后几何零位移**。
- **「改了背景却没声明字色」整类缺陷 15 处**：规则改了 `background` 但未同时声明 `color` → 继承到同选择器基规则的字色，与新底色不匹配。命中 **1.032～3.267** → **5.192～14.492**。含知识库对话框、待办对话框、番茄钟、桌面宠物对话、授权弹窗，以及四套主题的 `:pressed` 与聊天发送按钮禁用态。
- **奶油深色主题按钮文字几乎不可读**：`text_on_accent` 硬编码白字，而该主题深色档的强调实底是浅粉 `#FF9FB2` → 对比度 **1.935**。改为深字后 **8.794**（穷举 11 类落点全部 ≥4.5）。另三套深色板本来就用深字，奶油是唯一孤例。
- **聊天面板导出 / 停止按钮**：颜色是**构造期硬编码的内联样式**（这也是「换肤不跟随」的根因），已收口到唯一取色入口 `_apply_chat_theme()`。停止按钮字色 **2.775** → **6.071～6.131**；导出按钮描边 **1.588**（近乎不可见）→ **4.511～7.039**。
- **角色页右列没有滚动容器**（用户报「这些字又被挡住了」）：窗口高度不足时右列下半部分被裁掉。实测 `MainWindow.minimumSize` 为 1200×800 而页面 `minimumSizeHint` 需 898 → 角色页只拿到 739 却需 **837**，缺额被硬摊到页内控件：三个输入框被压到 **h=11**（`sizeHint` 35、字高 19 ⇒ 内容区 **−3，一个像素都画不出**），旁边三个标签同被压到 11（需 20）。给右列套滚动容器并补输入框最小高后：`minimumSizeHint` **837 → 261**、输入框 **11 → 36**（内容区 22 ≥ 墨迹 19）、滚到底部保存按钮可达、毛玻璃开/关两态黑占比 **0.0%**。
- **首页头像与当前角色不符**：头像只订阅了 `role_changed`，而该广播发生在 `gui/main.py:1600`、`MainWindow` 构造在 `:1633` ⇒ 首帧永远是过去时；且该订阅被包在 `if bridge is None: return` 里，桥接对象缺失时整条通道断掉。改为构造时主动拉取一次 + 拆成两条互不遮挡的通道 + 一次性防重连标记。取证：修前首页载入 `gui/assets/maid/normal.png`，修后与当前角色资产一致。
- **聊天浮窗 / 主面板的标签被刷上不透明底色**（用户报「左上角的那些字是单独的，跟背景不符」）：应用级 `QWidget{background-color:${bg}}` 会命中**裸 `QLabel`**，于是标题栏与提示标签各自画出一块不透明底，叠在毛玻璃浮窗上就是白/灰竖条。实测浮窗 4 处自带底色 **86.5 / 83.3 / 90.6 / 79.3% → 全部 0.0%**；同批横扫共 **9 处**（含顶栏状态标签 **100%** 在夜间主题上呈近白砖、群聊弹窗提示语）。
- **侧栏左下角「码铃·关系」chip 没有背景色**（用户报）：基态底色取 `bg_card`，而四套主题的侧栏根底**也是** `bg_card` → 同色 Δ=0，等于压根没画。改取 `bg_light`（悬停换 `surface_muted` 以免与基态撞色）。8 组实测通道差 **14～31**（浅色组 ≥17、深色组 ≥14），8/8 不再 Δ=0；字/底最低 11.556。
- **剩余「实底 + 方角」按钮**（用户报「把所有的方形纯色底按钮换成圆角」）：`§1f` 只覆盖了弹窗按钮与 `#sendBtn/#primaryBtn/#dangerBtn`；逐 id 渲染实测后四主题**一致**还有 **12 个/主题**（设置区「清除」×2、「检查更新」、「清理更新缓存」、`PageHelp`/`PageAbout`「返回首页」、`#checkUpdateBtn`、`#modelSaveBtn`、「➕ 新建角色」、`#planNewBtn`、`#styleSwitchBtn`×2）只吃到类型选择器的 `radius_pill`（24/24/24/26px）而控件高 28～33 ⇒ 半径超 `min(宽,高)/2` 被丢弃。新增 `§1g` 后**四主题实底方角 0 个**；逐按钮对账（改前/改后各导出一份 TSV，含判定/填充色/拐角切点/角像素四项）确认 **48 行差异全部且仅为「方角→圆角」**，宽/高/填充色全量零位移。
- **深色主题下主面板顶栏图标近乎不可见**：`icons.configure(app_ctx)` 在 `gui/main.py:1640`、**晚于** `MainWindow` 构造（`:1633`）⇒ 构造期出的图标按兜底色 `#000000` 绘制，并**写进 `QPixmapCache` 且无失效机制**，只有悬停或换肤才恢复。改为**显式按主题令牌传色**（未改动受保护的 `main.py` 启动顺序，理由已写在代码注释里）。
- **图标字形只在构造期求值一次**：字体资产的注册若晚于建窗，图标会**永久停在 emoji 态且换肤不恢复**（已用反事实复现：未注册字体时建窗 → 之后注册 + 两轮换肤 → 4 个文本一字未变）。抽出重取函数并挂到换肤点后，同样条件下恢复正常矢量字形；正常路径逐字不变。
- **浮窗左上角标题不随角色**（用户报「左上角的文字与全窗口不符」）：标题「与女仆的对话」是硬编码，`role_changed` 连了但只刷新了副标题，全文件 `titleLabel.setText` 调用点为 **0 处**。改为随当前角色刷新（**空名回退「码铃」**，不以人设标签当名字）。
- **浮窗右上角三键里的最小化键看不清**：三键字号不一致（最小化 20px、另两键 17px），且最小化键用的是 `−`(U+2212) 细横线 ⇒ 横向落墨仅 22px（同排 pin/attach 为 72/79~91px），视觉上最像空白格。已统一字号并让关闭键改走矢量图标族（manifest 无 minimize/maximize/subtract 语义名，故最小化/最大化**保留 Unicode 符号**——不硬塞语义不对的字形）。
- **「语音输入」点了像没反应**：该功能依赖可选包 `SpeechRecognition` + 麦克风，缺失时点击只弹一次说明就被误读为「没反应」。改为**依赖缺失即置灰 + tooltip 写明缺哪个包、如何安装**。
- **`_connect_signals` 脆弱耦合**：首行 `if chat_service is None: return` 会**连带跳过**主题切换、消息、角色、TTS 状态、情绪等与之无关的通道；且该函数**不幂等**（重复调用后同一次信号会触发多次）。改为「判空只保护真正依赖它的那一组」+ 每组一次性标记。反事实：`chat_service=None` 时改前 5 条通道各触发 **0** 次、容器 QSS 冻结在旧主题；改后各 **1** 次且跟随新主题；正常路径逐项零变化。
- **浮窗底部那行在窄窗把按钮压窄**：提示标签是裸 `QLabel`（既不省略也不换行 ⇒ `minimumSizeHint` 等于全文宽），使整行最小宽超出容器，`QBoxLayout` 摊派亏空后把三个扁平按钮各压窄 7px（内容区 41 < 文字宽 45）。改为让提示标签吃下剩余宽度并允许窄于全文宽后，最小窗宽 400 档按钮恒不被压。

### Changed
- **移除独立聊天浮窗的「表情 / 导出 / 语音」三个按钮**（用户指令）。同批删除随之**不可达**的表情面板及其 12 格按钮、`_on_emoji_clicked`、每次换肤对永不可见控件的 13 次样式设置，以及已无调用点的 `_flat_text_button_qss`。
  - **这三项功能保留在聊天主面板**（导出 `#exportChatBtn`、表情 `#quickActionBtn`、语音同为 `#quickActionBtn`）；「语音输入」另有全局入口（托盘菜单 → 跳转聊天页）。测试已改为把字形/裁切守卫指向主面板，并新增一条断言固化「浮窗不再有这三个按钮与表情面板」。
  - ⚠ 影响面：**只使用独立浮窗、不打开主窗**的用户，这三项需回主窗使用。
- `CHANNEL_URLS` 的 `stable` / `beta` 均指向 `master`（原 `main`）；同步更新 `tests/test_v20_check.py` 中**钉住分支名**的断言（不改则假红）。
- `gui/themes/base.qss` §1f / §1g 的圆角阈值注释**按实测重写**：判据为 `border-radius <= min(宽, 高) / 2`，**闭区间**（取等号仍圆角）；此前「该阈值已被证伪」的说法系误读 —— 那次反例实为控件**级内联**样式覆盖了主题层取值。同时订正 `radius_pill` 的遍历顺序与 `radius_sm` 取值（运行期实取 8/10/8/10）。

### Added
- **渲染级守卫测试**：`test_v22_1_contrast_qss.py` / `test_v22_1_chatbtn_tokens.py`（断言**实际渲染色**而非 setter 存值，含「样式只求值一次」的证伪用例）、`test_v22_1_blackbox_fixes.py`（69 例：焦点圈门控、定尺寸按钮内容区、毛玻璃切页、切片守卫、表情面板 retarget）、`test_v22_2_btn_radius.py`（30 例）、`test_v22_2_nav_icons.py`（16 例）、`test_v22_2_chatwindow_wiring.py`（5 例，含 **AST 级**「通道不得被 `chat_service` 判空圈住」与幂等/非空转断言）。
