...
a = Analysis(
    ...
    datas=[
        ...
        # v1.9 B(⚠-2): 内置字体子集 + OFL 许可副本。目标目录必须为 'assets/fonts'
        ...
        # 产物 dist_updater/maling_updater.exe（由 `pyinstaller updater.spec` 构建）。
        # datas 目标目录 'updater' → onedir 落在 <app>/_internal/updater/maling_updater.exe；
        # 与 gui/main.py:977 的 get_resource_path("updater")（frozen 态基准 sys._MEIPASS
    ...
    hiddenimports=[
        ...
        'reminders',
        'diary',
        # ---- v1.7.3: Pi 编程引擎试点（chat_service._run_pi_agent 动态 import）----
        'pi_backend',
        ...
        # ---- v1.9 B: 字体系统（theme_engine 顶部静态 import，防静态分析漏收）----
        'gui.fonts',
...
exe = EXE(
    ...
    icon='gui/assets/maling.ico',  # 应用图标
