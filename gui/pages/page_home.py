...
# 快捷入口定义：(key, 标题, 图标, tooltip, handler_key)
QUICK_ACTIONS = [
    ("chat", "开始聊天", "💬", "新开一段对话，和码铃说说话", "start_chat"),
    ("agent", "Agent", "🤖", "切换 Agent 模式：让码铃带工具干活", "agent"),
    ("tools", "工具箱", "🧰", "打开工具箱", "tools"),
    ("plan", "计划", "📋", "打开计划编辑器", "plan"),
    # v1.5.0: 知识库 / 待办 GUI 入口（审计 #C1/#C2 空壳销项）
    ("kb", "知识库", "📚", "索引本地文档并检索（与 CLI /kb 同一份索引）", "kb"),
    ("todo", "待办", "📝", "查看/添加/勾选待办事项（与 CLI /todo 同一份存档）", "todo"),
    # v1.4(A3): 番茄钟「她陪你专注」（D-V14-04 首页快捷入口）
    ("pomodoro", "专注", "🍅", "25 分钟番茄钟，她陪你专注", "pomodoro"),
    ("save", "保存会话", "💾", "保存当前会话", "save"),
    ("load", "加载会话", "📂", "加载已有会话", "load"),
    ("help", "帮助", "❓", "查看帮助", "help"),
    ("about", "关于", "🔔", "关于码铃", "about"),
]
...
class StatusCard(QFrame):
    ...
    def _init_ui(self, title: str, value: str, icon: str) -> None:
        layout = QVBoxLayout(self)
        layout.setContentsMargins(16, 16, 16, 16)
        layout.setSpacing(6)
...
class PageHome(QWidget):
    ...
    def _build_quick_actions(self) -> None:
        ...
        grid = QGridLayout()
        grid.setSpacing(10)
        for idx, (key, text, icon, tip, handler_key) in enumerate(QUICK_ACTIONS):
    ...
    def _build_mode_section(self) -> None:
        ...
        self.cards: dict = {}
        card_defs = [
            ("deep", "深度思考", "更详细的推理过程", "🧠", False),
            ("coding", "编程模式", "优先输出代码", "💻", False),
            ("chat", "闲聊模式", "弱化编程，自然对话", "💬", False),
            ("multi", "多模型", "启用多模型协作", "👥", False),
            ("stream", "流式输出", "实时显示 AI 回复", "📡", True),
        ]
        for i, (key, c_title, desc, icon, default) in enumerate(card_defs):
    ...
    def _refresh_model_entry(self) -> None:
        ...
        try:
            ...
            summary = None
        if not summary:
            self.model_entry_btn.setText("\u2699 模型设置")
            self.model_entry_hint.setText("")
            return
        if summary["configured"]:
            self.model_entry_btn.setText(
                f"\u2699 模型设置 · {summary['provider_short']} · {summary['masked_key']}"
            )
            hint = summary["state_text"]
        else:
            self.model_entry_btn.setText("\u2699 模型设置 · 未配置 Key，点此去配置")
